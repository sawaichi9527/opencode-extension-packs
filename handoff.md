...
| HEAD | `main`（2.0.15-dev：pwsh7 改為 Windows 主機強制替代、新增全域規則模板；前次 2.0.14：版號對齊 OpenCode 2.0.14；實際 SHA 以 `git log -1` 為準） |
