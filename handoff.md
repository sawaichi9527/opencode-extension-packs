...
| 項目 | 值 |
|---|---|
| 版本 | `2.0.15`（`VERSION` / `manifest/packs.json`；基於 OpenCode v2.0.14 驗證） |
| HEAD | `main`（2.0.15：pwsh7 改為 Windows 主機強制替代、新增全域規則模板；註冊 `token-monitor` external-tool pack 並完成 **Ubuntu desktop 驗證**；前次 2.0.14：版號對齊 OpenCode 2.0.14；實際 SHA 以 `git log -1` 為準） |
...
- `manifest/packs.json` — 套件清單，schemaVersion 1，version 2.0.15
...
- `token-monitor` → `Javis603/token-monitor@v0.60.0`（`external-tool`；Windows 10 IoT Enterprise LTSC 2021（無 WSL）已驗證 portable／NSIS／headless 三路徑；**Ubuntu desktop（x64, X11, glibc）已驗證 AppImage**）
